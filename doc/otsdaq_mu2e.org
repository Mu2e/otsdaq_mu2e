...
** ARTDAQ                                                                    
   intro by Eric: https://mu2e-docdb.fnal.gov/cgi-bin/sso/ShowDocument?docid=45544
...
** DCS : slow controls (Detector Control System)                             
  ...
  - pull otsdaq_utilities from development https://github.com/art-daq/otsdaq_utilities.git
...
*** Restarting the PDU EPICS IOC server                                      
- could solve the wrong starting of it after a rebooting or switching on the mu2-dcs-01 node. 
  Steps to follow (https://dbweb9.fnal.gov:8443/ECL/mu2e/E/show?e=1806)
#+begin_src 
 ssh mu2edcs@mu2e-dcs-01.fnal.gov 
 screen -ls               # shows if the PDUIoc screen is up
 screen -r PDUIoc         # then write the command 'exit' (it closes the screen) 
                          #  wait for 2 minutes 
 screen -ls               # to check if the PDUIoc is up again 
 screen -r PDUIoc         # again to check if there are still starting errors 
                          # then press the keys 'ctrl' and 'A' and then 'D' to detach the screen without closing it
#+end_src 
*** Glenn in mu2ewiki: [[https://mu2einternalwiki.fnal.gov/wiki/Detector_Control_System]]
** [[file:mongo_db.org][Mongo DB and the DB tables]]
** [[file:postgres_db.org][online postgres DB]]
* ------------------------------------------------------------------------------
* ATTIC
* ------------------------------------------------------------------------------
