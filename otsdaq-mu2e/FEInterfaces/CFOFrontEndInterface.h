...
class CFOFrontEndInterface : public CFOandDTCCoreVInterface
{
  public:
	...
	// CFO specific items
	//----------------
	float MeasureLoopback(
	    int linkToLoopback);  ///< pre-covid loopback calculation stretegy
	...
	virtual mu2edev* getDevice(void) override
	{
		if(!thisCFO_)
		{
			__SS__ << "thisCFO_ pointer has not been initialized! "
	...
	static void handleDetachedSubevent(
	    const CFOLib::CFO_Event& subevent,
	    std::shared_ptr<CFOFrontEndInterface::DetachedBufferTestThreadStruct>
	        threadStruct);
  ...
  private:
	...
	void parseEventDurationForRunPlan(const std::string& eventDuration,
	                                  std::string&       durationValue,
	                                  std::string&       durationUnits);
	void getRatioOfOnPerEvents(uint32_t  clocksPerOn,
	                           uint32_t  clocksPerEvent,
	                           uint32_t& mPartRatio,
	                           uint32_t& nPartRatio);
	void generateSharedRunPlanWithPeriodicModeOn(
	...
	const std::vector<uint32_t> standardNValues_                = {
        100, 200, uint32_t(1e3), uint32_t(1e5), uint32_t(1e7), uint32_t(1e9)};
  ...
  public:
	...
	std::string CompileSetAndLaunchTemplateSuperCycleRunPlan(
	    bool     enable,
	...
	void SharedRunPlanStatus(
	    __ARGS__);  ///< Get Event Mode, Tag, Active Subsystems, and running status
