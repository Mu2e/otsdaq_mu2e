...
void CFOFrontEndInterface::SharedRunPlanSubsystemSingleShotJoin(__ARGS__)
{
	...
	{
		__FE_SS__
		    << "Single-shot duty cycle currently supports resolved N <= "
	...
	if(chunkCounts.size() > 1)
	{
		result << "Requested single-shot count " << singleShotCount
		       << " with duty " << mPartRatio << ":" << nPartRatio
	...
	if(!isFullDutyCycle)
		result << "Duty is below 100%; single-shot chunking is constrained to fine batches "
	...
	auto configureSingleShotChunk = [&](uint32_t               chunkIndex,
		...
		if(chunkIndex >= chunkCoarseMaskIndices.size())
		{
			__FE_SS__ << "Internal error: invalid single-shot chunk index " << chunkIndex
			          << " (chunkCoarseMaskIndices size="
			          << chunkCoarseMaskIndices.size() << ")." << __E__;
			__FE_SS_THROW__;
		}
		...
		if(coarseMaskIndex >= 0)
		{
			...
			if(coarseEndAndIdx >= chunkAndMasks.size())
			{
				__FE_SS__ << "Internal error: coarse end-AND index out of range "
				          << coarseEndAndIdx << " (andMasks size="
				          << chunkAndMasks.size() << ")." << __E__;
				__FE_SS_THROW__;
			}
			...
			chunkLog << "Using coarse-batch chunk N=" << chunkCount
			         ...
			         << "): keep forced at all AND points, clear applied only at coarse end-AND index "
		...
		else
		{
			...
			{
				__FE_SS__ << "Internal error: unsupported duty-based fine-batch chunk count "
			...
			chunkLog << "Using duty-based fine-batch chunk N=" << chunkCount
			         ...
			         << " fine locations, AND clears applied at all loop AND points." << __E__;
		}
	...
	for(size_t chunkIndex = 0; chunkIndex < chunkCounts.size(); ++chunkIndex)
	{
		...
		configureSingleShotChunk(
		    chunkIndex, chunkCounts[chunkIndex], chunkAndMasks, singleShotMasks, subResult);
...

		{
			...
			if(chunkIndex == 0)
				result << "Set preparation Run Plan for single-shot chunk "
			       << (chunkIndex + 1) << " (with " << chunkCounts.size() - 1 << " chunks to follow)"				   
...

		{
			...
			subResult << "Set armed Run Plan for single-shot chunk " << (chunkIndex + 1)
    ...
    const bool                   applyPeriodicOrMasks)
{
	...
	if(singleShotMasks.size() &&
	   singleShotMasks.size() != N_coarse + standardNValues_[0])
	{
		__FE_SS__ << "singleShotMasks must have size "
		          << (N_coarse + standardNValues_[0])
