...
class ROCPolarFireCoreInterface : public ROCCoreVInterface
{
  public:
	...
	virtual void     writeEmulatorRegister(DTCLib::roc_address_t address,
	                                       DTCLib::roc_data_t    data_to_write) override;
	...
	bool isActionDone(DTCLib::roc_data_t* readStatus        = nullptr,
	...
	void writeSPIFlashBlock(const std::vector<uint16_t>& writeData,
	                        uint32_t                     startAddress,
	                        bool                         waitForDone = true) override;
	void eraseSPIFlashBlock(uint32_t eraseSize,
	                        uint32_t startAddress,
	                        bool     waitForDone = true) override;
