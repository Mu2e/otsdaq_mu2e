...
ROCTimingPaddlesInterface::ROCTimingPaddlesInterface(
	...
	registerFEMacroFunction("Setup For Run",
		static_cast<FEVInterface::frontEndMacroFunction_t>(
			&ROCTimingPaddlesInterface::PrepareDataRuns),
		std::vector<std::string>{
					"Enable Delay (Default := true)",
					"Delay amount (Default := 0)"
		},  // inputs parameters
		std::vector<std::string>{},  // output parameters
		1							 // requiredUserPermissions
	);

	registerFEMacroFunction("Selective Reset",
		static_cast<FEVInterface::frontEndMacroFunction_t>(
			&ROCTimingPaddlesInterface::SelectiveReset),
		std::vector<std::string>{
					"40MHz Clock Generator (Default := false)",
	...
	registerFEMacroFunction("Reset Histograms",
		static_cast<FEVInterface::frontEndMacroFunction_t>(
		...
		std::vector<std::string>{
			...
			"Packet Sender Loopback Markers(Default := false)"
		},  // inputs parameters
		std::vector<std::string>{},  // output parameters
		1							 // requiredUserPermissions
	);

	registerFEMacroFunction("varTest",
		static_cast<FEVInterface::frontEndMacroFunction_t>(
	...
	registerFEMacroFunction("BERT Setup",
		static_cast<FEVInterface::frontEndMacroFunction_t>(
		...
		std::vector<std::string>{
			...
			"External loopback mode (Default := false)"
		},  // inputs parameters
		std::vector<std::string>{},  // output parameters
		1							 // requiredUserPermissions
	);
...
void ROCTimingPaddlesInterface::configure(void) try {
...
//==================================================================================================
bool ROCTimingPaddlesInterface::running(void) {
...
void ROCTimingPaddlesInterface::varTest(__ARGS__) {
	...
	histogramTemplate = std::regex_replace(histogramTemplate, std::regex("<LOOPBACK>"), data);
	histogramTemplate = std::regex_replace(histogramTemplate, std::regex("<CLOCK>"), data);
	histogramTemplate = std::regex_replace(histogramTemplate, std::regex("<EVENT>"), data);

	histogramTemplateTest = std::regex_replace(histogramTemplateTest, std::regex("<LOOPBACK>"), data);
	histogramTemplateTest = std::regex_replace(histogramTemplateTest, std::regex("<CLOCK>"), dataTest);
...
//==================================================================================================
std::vector<uint16_t> ROCTimingPaddlesInterface::readHistogram(std::vector<DTCLib::roc_data_t>& histAddrs, uint32_t loc_addr, std::stringstream& outss) 
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		outss << std::left << std::setw(12) << label
...
//======================================================================================================
void ROCTimingPaddlesInterface::Reset(__ARGS__)
{
	writeRegister(0, 0);
}
...
void ROCTimingPaddlesInterface::GetStatus(__ARGS__)
{
	...
	outss << "Last Data Request Packet: " <<
		std::hex << getDTC()->ReadExtROCRegister(linkID_, 9, 5) << 
		std::hex << getDTC()->ReadExtROCRegister(linkID_, 9, 4) <<
		std::hex << getDTC()->ReadExtROCRegister(linkID_, 9, 5) << __E__;

	outss << "Number of Heart Beat Packets: " << getDTC()->ReadExtROCRegister(linkID_, 9, 22) << __E__;
	outss << "Number of Data Request Packets: " << getDTC()->ReadExtROCRegister(linkID_, 9, 23) << __E__;
...
void ROCTimingPaddlesInterface::BERT(__ARGS__)
{
	...
	if (fixedPatternEnabled) getDTC()->WriteExtROCRegister(linkID_, 13, 6, 0, 0, 1000) ;
	else getDTC()->WriteExtROCRegister(linkID_, 13, 7, 0, 0, 1000);

	DTCLib::roc_data_t fixedPattern = __GET_ARG_IN__("Fixed Pattern (Default := 0xBC3C)", DTCLib::roc_data_t, 0xBC3C);
	getDTC()->WriteExtROCRegister(linkID_, 13, 3, fixedPattern, 0, 1000);

	DTCLib::roc_data_t fixedKChar = __GET_ARG_IN__("Fixed K Char (Default := 0X3)", DTCLib::roc_data_t, 0x3);
	getDTC()->WriteExtROCRegister(linkID_, 13, 2, fixedKChar, 0, 1000);

	DTCLib::roc_data_t loopbackModeEnabled = __GET_ARG_IN__("Loopback Mode (Default := false)", bool, false);
