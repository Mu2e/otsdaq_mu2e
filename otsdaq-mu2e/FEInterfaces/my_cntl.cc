...
int main(int argc, char* argv[])
{
	...
	// 	  reg_access.access_type = 1;//wr
	// 	  reg_access.val         = 0x100; // bit 8 is DMA Enable (offset 0x2***
	// is for C2S chan 0) 	  sts = ioctl( fd, M_IOC_REG_ACCESS, &reg_access
	// ); 	  if (sts) { perror("ioctl M_IOC_REG_ACCESS write"); return (1);
	// }
	...
	// ); 	  data.hdr.s.Valid = 1; 	  data.hdr.s.PacketType = 5;  //
	// could be overwritten below 	  data.hdr.s.RingID = 0;
	// 	  data.hdr.s.PacketCount = (data.hdr.s.TransferByteCount-16)/16;// minus
	// header, / packet size
