...
try
{
	...
	if(argc != 4)
	{
		__COUT_ERR__ << "\n\n\tUsage = Need 3 arguments: CFOFrontEndInterface_TestMain "
		                "<deviceIndex> <numberOfEventWindowMarkers> <runPlanMode>\n\n."
		             << __E__;
		__COUT_INFO__ << "\n\n\t\tUsage = <numberOfEventWindowMarkers> -1:   CFO Reset \n"
		              << "\n\n\t\tUsage = <numberOfEventWindowMarkers> -2:   JA Reset \n"
		              << "\n\n\t\tUsage = <runPlanMode> 0-2:   no timing markers and yes "
		                 "read CFO records -- 0:1.7us, 1:supercycles 2:100us\n"
		              << "\n\n\t\tUsage = <runPlanMode> 3-5:  yes timing markers and yes "
		                 "read CFO records -- 0:1.7us, 1:supercycles 2:100us\n"
		              << "\n\n\t\tUsage = <runPlanMode> 6-8:   no timing markers and  no "
		                 "read CFO records -- 0:1.7us, 1:supercycles 2:100us\n"
		              << "\n\n\t\tUsage = <runPlanMode> 9-11: yes timing markers and  no "
		                 "read CFO records -- 0:1.7us, 1:supercycles 2:100us\n";
		return 0;
	}
	...
	// // Variables
	std::string supervisorContextUID_     = "FEContext_CFO"; //"ContextCalo03";
	std::string supervisorApplicationUID_ = "FESupervisor_CFO"; //"FESupervisorCalo03";
	std::string feUID_ =
	    deviceIndex == 0 ? "CFO_0": "CFO_1"; //"DTC4" : "DTC5";  //DTC4 for Device0 and DTC5 for Device1
	...
	if(numberOfEventWindowMarkers == uint32_t(-1))
	{
		...
		cfo.halt();
		cfo.getCFOandDTCRegisters()->SetJitterAttenuatorSelect(1 /* select RJ45 */,
		                                                       false /* alsoResetJA */);
		sleep(1);
		__COUT_INFO__ << "JA Status = "
		              << cfo.getCFOandDTCRegisters()->FormatJitterAttenuatorCSR()
		              << __E__;
	...
	if(numberOfEventWindowMarkers == uint32_t(-3))
	{
		...
		__COUT_INFO__ << "CFO DMA sizes = " << cfo.thisCFO_->FormatDMATransferLength()
		              << __E__;
		return 0;
	}
	...
	__COUT_INFO__ << "JA Status = "
	              << cfo.getCFOandDTCRegisters()->FormatJitterAttenuatorCSR() << __E__;
	...
	else if(runPlanMode == 1)
	{
		...
		numberOfEventWindowMarkers *=
		    245000;  //set event cout expectation for check below (235K on-spill + 10K off-spill per cycle)
	}
	...
	while(1)
	{
		...
		if(i % 10 == 9)
		{
			...
			__COUT_INFO__ << "\n"
			              << CFOFrontEndInterface::getDetachedBufferTestStatus(
			                     cfo.bufferTestThreadStruct_)
			              << __E__;
		}

		if((i > 5 && !cfo.bufferTestThreadStruct_->running_) ||
		   CFOFrontEndInterface::getDetachedBufferTestReceivedCount(
		       cfo.bufferTestThreadStruct_) >=
		       numberOfEventWindowMarkers - 1)  // start mutex scope
		{
			__COUT_INFO__ << "Iteration exit #" << i << " - thread running = "
			              << cfo.bufferTestThreadStruct_->running_ << "\n"
			              << CFOFrontEndInterface::getDetachedBufferTestStatus(
			                     cfo.bufferTestThreadStruct_)
			              << __E__;
			...
			if(CFOFrontEndInterface::getDetachedBufferTestReceivedCount(
			       cfo.bufferTestThreadStruct_) < numberOfEventWindowMarkers - 1)
				dumpSpy = true;
			break;
		}
		++i;
