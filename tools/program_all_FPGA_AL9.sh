...
if [[ "x$1" == "xNORESET" ]]; then
    echo -e "$(date +%d%b%y.%T) program_all_FPGA_AL9.sh:${LINENO} |  \t No reset!"
    DORESET=0
    shift
fi
...
NO_OTS_KILL=0
if [[ "x$1" == "xNOKILL" ]]; then
    echo -e "$(date +%d%b%y.%T) program_all_FPGA_AL9.sh:${LINENO} |  \t No-ots-kill specified."
    NO_OTS_KILL=1
    shift
fi
...
DRYRUN=$2
if [ ! -f $BITFILE ]; then
    echo -e "$(date +%d%b%y.%T) program_all_FPGA_AL9.sh:${LINENO} |  \t No bit file ${BITFILE} found"
    exit 1
fi
...
echo -e "$(date +%d%b%y.%T) program_all_FPGA_AL9.sh:${LINENO} |  \t ===> Done with ${HOSTNAME}. Found ${INDEX} JTAGs, programmed  bitfile and PCIe reset for each!"
